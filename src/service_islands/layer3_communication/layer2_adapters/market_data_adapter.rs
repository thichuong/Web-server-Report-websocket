...
// use crate::service_islands::layer3_communication::layer2_grpc_client::Layer2GrpcClient; // Not used
use crate::service_islands::layer1_infrastructure::cache_system_island::{CacheSystemIsland, CacheStrategy};

/// Market Data Adapter (gRPC-Based, Cache-Optimized)
///
/// Handles all Layer 3 → Layer 2 market data communication via gRPC.
...
/// to minimize unnecessary Layer 2 calls. Uses gRPC for 50-70% better performance vs HTTP.
...
impl MarketDataAdapter {
    ...
    pub fn new() -> Self {
        Self {
            layer2_grpc_client: None,
            cache_system: None,
        }
    }

    /// Set Layer 2 gRPC Client dependency (High Performance)
    ...
    pub async fn health_check(&self) -> bool {
        if let Some(grpc_client) = &self.layer2_grpc_client {
            match grpc_client.health_check().await {
                Ok(_) => {
                    println!("  ✅ Market Data Adapter - Layer 2 gRPC service healthy");
                    true
                }
                Err(e) => {
                    ...
                    if error_msg.contains("429") || error_msg.contains("Circuit breaker") || error_msg.contains("rate limit") {
                        println!("  ⚠️ Market Data Adapter - Layer 2 gRPC service rate limited (functional)");
                        true
                    } else {
                        println!("  ❌ Market Data Adapter - Layer 2 gRPC service unhealthy: {}", e);
                        false
                    }
                }
            }
        } else {
            println!("  ⚠️ Market Data Adapter - Layer 2 gRPC service not configured");
            true // Not configured is not an error
        }
    }

    /// Check if Layer 2 gRPC is configured
    pub fn is_layer2_configured(&self) -> bool {
        self.layer2_grpc_client.is_some()
    }
    ...
    pub async fn fetch_dashboard_summary_v2(&self, force_realtime_refresh: bool) -> Result<serde_json::Value> {
        ...
        let grpc_client = self.layer2_grpc_client.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Layer 2 gRPC Client not configured in MarketDataAdapter"))?;

        // Handle force refresh: bypass cache completely
        if force_realtime_refresh {
            if let Some(cache_system) = &self.cache_system {
                println!("🔄 [Layer 3] Force refresh - bypassing cache and calling Layer 2 service via gRPC");
                let layer2_data = grpc_client.fetch_dashboard_summary_v2(force_realtime_refresh).await?;
        ...
        if let Some(cache_system) = &self.cache_system {
            let grpc_client_clone: Arc<String> = Arc::clone(grpc_client);
            let cache_system_clone: Arc<CacheSystemIsland> = Arc::clone(cache_system);

            match cache_system.cache_manager().get_or_compute_typed(
                "latest_market_data",
                CacheStrategy::RealTime, // 30s TTL for real-time data
                || async move {
                    println!("🔄 [Layer 3 → Layer 2] Fetching dashboard summary from Layer 2 service via gRPC...");
                    let layer2_data = grpc_client_clone.fetch_dashboard_summary_v2(false).await?;
                    ...
                    println!("✅ [Layer 3] Fresh data fetched from Layer 2 service via gRPC");
                    Ok(layer2_data)
                }
            ).await {
                Ok(data) => {
            ...
            // No cache system - fallback to direct gRPC call
            println!("⚠️ [Layer 3] No cache system - calling Layer 2 service directly via gRPC");
            grpc_client.fetch_dashboard_summary_v2(force_realtime_refresh).await
        }
    }
    ...
    #[allow(dead_code)]
    pub async fn fetch_and_stream_dashboard(&self) -> Result<serde_json::Value> {
    ...
    pub fn supports_cache_free_mode(&self) -> bool {
        let layer2_support = if let Some(_grpc_client) = &self.layer2_grpc_client {
            // Layer2 gRPC service supports cache-free mode
            true // Layer2 Data Service supports cache-free mode
        } else {
            false
        };
        ...
        println!("  - Layer 2 gRPC service configured: {}", layer2_support);
